...
4. `user-agent: claude-cli/2.1.280 (external, sdk-cli)` — injected; Anthropic enforces per-model Claude Code minimum versions on this header (bumped 2026-09-23, see `docs/audits/header-provenance.md`)
5. `anthropic-version: 2023-06-01` — injected
6. `x-anthropic-billing-header` — **not sent** (retired 2026-09-23; client-supplied values are stripped). Not used by the server for the per-model version floor, and not required for acceptance per the D003 A/B
7. System prompt prefix: "You are Claude Code..." — injected for all requests with a string `model` field

Header provenance caveat: a Claude Code v2.1.132 local MITM capture saw the billing-header debug attribution line but did not see `x-anthropic-billing-header` on the actual `/v1/messages` request. As of 2.1.280 genuine Claude Code sends the attribution string as the first `system` block with conversation-derived `cch` and version suffix, not as a header. The proxy's header injection was removed on 2026-09-23 after a present/absent A/B returned 200 both ways (Lab Operations `incidents/2026/INC-2026-001/references/D003-attribution-decision.md`); mirroring the body-block fingerprint is rejected until something observably requires it.
