...
| `user-agent` | `claude-cli/2.1.280 (external, sdk-cli)` | Captured from Claude CLI traffic; the server enforces per-model Claude Code minimum versions on this header. See `docs/audits/header-provenance.md` |
| `anthropic-version` | `2023-06-01` | API version |
| `x-anthropic-billing-header` | *(not sent; stripped if client-supplied)* | Injected May–September 2026 as an attribution marker. Retired 2026-09-23: genuine Claude Code never sends it as a header (2.1.280 sends the string as the first `system` block), and a present/absent A/B returned 200 both ways. See `docs/audits/header-provenance.md`. |

Client-provided `anthropic-beta` values are merged (deduplicated) with the required set. No billing attribution is sent: the server does not use it for the per-model Claude Code version floor (that is `user-agent`), genuine Claude Code 2.1.280 sends the attribution as the first `system` block rather than a header with conversation-derived digests the proxy does not mirror, and the pre-specified present/absent A/B returned 200 both ways. Decision and evidence: Lab Operations `incidents/2026/INC-2026-001/references/D003-attribution-decision.md` (2026-09-23).
...
- 2026-09-23: the injection was removed after the pre-specified present/absent A/B returned 200 both ways (Lab Operations `incidents/2026/INC-2026-001/references/D003-attribution-decision.md`). The proxy strips any client-supplied `x-anthropic-billing-header`. The body-block attribution genuine Claude Code sends is not mirrored.
